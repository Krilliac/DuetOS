...
| **settings** | [`settings.cpp`](../../kernel/apps/settings.cpp) (+ `settings_datetime`, `settings_display`, `settings_keyboard`, `settings_mouse`, `settings_sound`) | Multi-page settings panel: time + display theme + input + audio. Sound page has a master-volume level bar (`+`/`-` ±5%, `V` mute) backed by the audio backend's software gain stage; level + mute persist via `SESSION.CFG`. | time, gpu, input, audio |
