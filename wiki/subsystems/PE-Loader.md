...
4. **Preload set**: register every userland DLL into the per-process
   DLL table (`Process::dll_images[]`). 46 DLLs preloaded out of the
   47 production DLLs in `userland/libs/` (only `vulkan_1.dll`
   loads on demand); ~1100 exports total. Per-DLL status lives in
