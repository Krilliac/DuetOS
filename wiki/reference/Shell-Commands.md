...
Source: `kernel/shell/*.cpp` — every `void Cmd*(...)` symbol appears here. Total: **252** handlers across **19** TUs.
...
| `kernel/shell/shell_hardware.cpp` | `CmdCpuFeatures`, `CmdCpuid`, `CmdCr`, `CmdDpms`, `CmdFb`, `CmdGfx`, `CmdGpu`, `CmdHdaJacks`, `CmdHeap`, `CmdHeapLeaks`, `CmdHeapLeaksWatch`, `CmdHpet`, `CmdHw`, `CmdHwmon`, `CmdKbdStats`, `CmdLapic`, `CmdLspci`, `CmdMei`, `CmdMonitor`, `CmdMouseStats`, `CmdMsr`, `CmdPaging`, `CmdPower`, `CmdRflags`, `CmdSmbios`, `CmdSmp`, `CmdThermal`, `CmdTicks`, `CmdTsc`, `CmdVbe`, `CmdVtop` |
