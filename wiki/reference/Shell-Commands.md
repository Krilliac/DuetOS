...
The handler-symbol inventory below is auto-derived from
`kernel/shell/*.cpp`; the prose categories that follow are
maintained by hand and capture per-command semantics that the
symbol grep can't see.

<!-- AUTO:shell-commands START -->
<!-- generated by tools/build/gen-wiki-auto.py — do not edit by hand -->

Source: `kernel/shell/*.cpp` — every `void Cmd*(...)` symbol appears here. Total: **270** handlers across **26** TUs.

| Source | Handlers |
|--------|----------|
| `kernel/shell/shell_bench.cpp` | `CmdBench` |
| `kernel/shell/shell_bootslot.cpp` | `CmdBootslot`, `CmdSlotinfo` |
| `kernel/shell/shell_core.cpp` | `CmdAbout`, `CmdAlias`, `CmdClear`, `CmdConsole`, `CmdDate`, `CmdEnv`, `CmdFalse`, `CmdGetenv`, `CmdHostname`, `CmdPanicTest`, `CmdPwd`, `CmdSet`, `CmdTrue`, `CmdUnalias`, `CmdUname`, `CmdUnset`, `CmdUptime`, `CmdVersion`, `CmdWhoami`, `CmdYield` |
| `kernel/shell/shell_dbg.cpp` | `CmdDbg` |
| `kernel/shell/shell_debug.cpp` | `CmdAddr2Sym`, `CmdBp`, `CmdCapAuditMode`, `CmdDomain`, `CmdDomainRestart`, `CmdDumpState`, `CmdHealth`, `CmdInspect`, `CmdInspectArm`, `CmdInspectCapAudit`, `CmdInspectCounters`, `CmdInspectDomains`, `CmdInspectEntropy`, `CmdInspectFaultReact`, `CmdInspectGdb`, `CmdInspectHelp`, `CmdInspectIpc`, `CmdInspectLockdep`, `CmdInspectOpcodes`, `CmdInspectRcu`, `CmdInspectSecurity`, `CmdInspectSyscalls`, `CmdInspectSyscallsCaps`, `CmdInspectThreads`, `CmdInspectTracerStats`, `CmdInspectUptime`, `CmdInspectZones`, `CmdInstr`, `CmdLockdepPanic`, `CmdMemDump`, `CmdModule`, `CmdPerf`, `CmdPerfDump`, `CmdProbe`, `CmdTrace`, `CmdTracer`, `CmdTracerDump`, `CmdTracerKind` |
| `kernel/shell/shell_diag.cpp` | `CmdDfix`, `CmdDintro` |
| `kernel/shell/shell_dispatch.cpp` | `CmdHelp`, `CmdRepeat`, `CmdSource`, `CmdSysinfo`, `CmdTactility`, `CmdTheme`, `CmdTime`, `CmdWhich`, `CmdWindows` |
| `kernel/shell/shell_display.cpp` | `CmdAutonomic`, `CmdDpms`, `CmdFbdump`, `CmdGfx`, `CmdGpu`, `CmdHdaJacks`, `CmdMePsp`, `CmdMei`, `CmdMonitor`, `CmdNpu`, `CmdVbe` |
| `kernel/shell/shell_drsh.cpp` | `CmdDrshd` |
| `kernel/shell/shell_exec.cpp` | `CmdExec`, `CmdLinuxexec`, `CmdPeTriage`, `CmdPeexec`, `CmdRead`, `CmdReadelf`, `CmdTranslate`, `CmdUnzip` |
...
| `kernel/shell/shell_filesystem.cpp` | `CmdCat`, `CmdCp`, `CmdEcho`, `CmdFatappend`, `CmdFatcat`, `CmdFatls`, `CmdFatmkdir`, `CmdFatnew`, `CmdFatrm`, `CmdFatrmdir`, `CmdFattrunc`, `CmdFatwrite`, `CmdFind`, `CmdGrep`, `CmdHead`, `CmdLs`, `CmdMv`, `CmdRm`, `CmdSort`, `CmdTail`, `CmdTouch`, `CmdUniq` |
| `kernel/shell/shell_hardware.cpp` | `CmdCpuFeatures`, `CmdCpuid`, `CmdCr`, `CmdFb`, `CmdHeap`, `CmdHeapLeaks`, `CmdHeapLeaksWatch`, `CmdHpet`, `CmdHw`, `CmdHwmon`, `CmdIo`, `CmdKbdStats`, `CmdLapic`, `CmdLspci`, `CmdMouseStats`, `CmdMsr`, `CmdPaging`, `CmdPeek`, `CmdPoke`, `CmdPower`, `CmdRflags`, `CmdSmbios`, `CmdSmp`, `CmdThermal`, `CmdTicks`, `CmdTsc`, `CmdVtop`, `CmdWrmsr` |
| `kernel/shell/shell_kpath.cpp` | `CmdKpath` |
| `kernel/shell/shell_leakcheck.cpp` | `CmdLeakCheck` |
| `kernel/shell/shell_live_update.cpp` | `CmdLiveUpdate` |
| `kernel/shell/shell_loadtest.cpp` | `CmdLoadTest` |
| `kernel/shell/shell_logging.cpp` | `CmdFaultInject`, `CmdKdbg`, `CmdLogarea`, `CmdLogclock`, `CmdLogcolor`, `CmdLoglevel`, `CmdMetrics` |
| `kernel/shell/shell_network.cpp` | `CmdArp`, `CmdBt`, `CmdCrTrace`, `CmdDhcp`, `CmdFirewall`, `CmdFwPolicy`, `CmdFwTrace`, `CmdHttp`, `CmdIfconfig`, `CmdIpv4`, `CmdNet`, `CmdNetscan`, `CmdNic`, `CmdNslookup`, `CmdNtp`, `CmdPing`, `CmdRoute`, `CmdUsbNet`, `CmdWifi` |
| `kernel/shell/shell_process.cpp` | `CmdAffinity`, `CmdFree`, `CmdKill`, `CmdPs`, `CmdResmon`, `CmdResume`, `CmdSpawn`, `CmdSuspend`, `CmdTop` |
| `kernel/shell/shell_security.cpp` | `CmdAttackSim`, `CmdCaplog`, `CmdElevate`, `CmdElevations`, `CmdGuard`, `CmdIdleLock`, `CmdLoginCmd`, `CmdLogout`, `CmdPasswd`, `CmdPolicy`, `CmdPurple`, `CmdRoleAdd`, `CmdRoleDel`, `CmdRoles`, `CmdSecEvents`, `CmdSu`, `CmdUnlock`, `CmdUseradd`, `CmdUserdel`, `CmdUsers` |
| `kernel/shell/shell_selfthink.cpp` | `CmdSelfthink` |
| `kernel/shell/shell_service.cpp` | `CmdSvc` |
| `kernel/shell/shell_storage.cpp` | `CmdFstrim`, `CmdInstall`, `CmdLastdump`, `CmdLsblk`, `CmdLsgpt`, `CmdLsmod`, `CmdMkfs`, `CmdMkfsDuetfs`, `CmdMount` |
| `kernel/shell/shell_utilities.cpp` | `CmdBasename`, `CmdBeep`, `CmdCal`, `CmdChecksum`, `CmdColor`, `CmdDirname`, `CmdDmesg`, `CmdExpr`, `CmdFlushTlb`, `CmdHexdump`, `CmdHistory`, `CmdMan`, `CmdMem`, `CmdMode`, `CmdNl`, `CmdRand`, `CmdReset`, `CmdRev`, `CmdSeq`, `CmdSleep`, `CmdStat`, `CmdStats`, `CmdTac`, `CmdUuid` |
| `kernel/shell/shell_wget.cpp` | `CmdBase64`, `CmdCmp`, `CmdCrc32`, `CmdDd`, `CmdSha256Sum`, `CmdTee`, `CmdTr`, `CmdWc`, `CmdWget`, `CmdXxd` |
<!-- AUTO:shell-commands END -->
...
- [Danger zone — raw hardware / memory pokes](#danger-zone--raw-hardware--memory-pokes)
...
| `caplog [count]` | Newest-first dump of the cap-gate denial ring (sequence, tick, pid, syscall #, missing cap). Counts of total calls / denies / dropped-from-ring print as a header. |
...
| `suspend <tid>` | Bump a task's NT-style suspend count (admin) | Lazy park at next schedule |
| `resume <tid>` | Drop a task's suspend count; re-ready at zero (admin) | |
| `affinity <tid> <cpu>` | Hint the task's next wake onto a CPU (admin) | Hint, not a hard pin |
...
| `vtop <vaddr>` | Walk active CR3: VA → PA + PTE flag decode (admin) |
...
| `dpms [on\|standby\|suspend\|off]` | Query / set VESA monitor power state |
...
| `logclock [on\|off]` | Get / set klog wall-clock prefix | |
...
These are the kernel-bring-up power tools (the `wrmsr` / `devmem2` /
`inb`/`outb` equivalents). They bypass **every** kernel safety net by
design: no MSR whitelist, no page-permission check, no device
arbitration, no undo. A wrong value triple-faults the box or silently
corrupts RAM — that is the accepted cost of the tool, not a bug.

Every one is **(admin)**-gated in the dispatcher **and** prints a
risk banner and refuses to act unless the literal token `FORCE` is
appended (same contract as `mkfs … ERASE`). The two read-only probes
(`peek`, `io in*`) are admin-only but need no `FORCE`; they are still
kernel info-leak oracles.

| Command | Synopsis | Guard |
|---|---|---|
| `wrmsr <hidx> <hval> FORCE` | Write a model-specific register (no whitelist) | admin + FORCE |
| `io inb\|inw <port>` | Raw x86 port read | admin |
| `io outb\|outw <port> <val> FORCE` | Raw x86 port write | admin + FORCE |
| `peek <hpaddr> [b\|w\|d\|q]` | Read raw physical RAM via the 1 GiB direct map | admin |
| `poke <hpaddr> <hval> [b\|w\|d\|q] FORCE` | Write raw physical RAM | admin + FORCE |

Addresses/values are hex (bare or `0x`). `peek`/`poke` refuse
addresses outside the 1 GiB direct map (a `PhysToVirt` panic
otherwise). There is intentionally no `peek`-style guard on `wrmsr`
or `io out` — picking the right register/port is the operator's
responsibility, which is why the banner exists.
