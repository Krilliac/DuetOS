...
Source: `kernel/shell/*.cpp` — every `void Cmd*(...)` symbol appears here. Total: **224** handlers across **16** TUs.

| Source | Handlers |
|--------|----------|
| `kernel/shell/shell_bench.cpp` | `CmdBench` |
| `kernel/shell/shell_core.cpp` | `CmdAbout`, `CmdAlias`, `CmdClear`, `CmdDate`, `CmdEnv`, `CmdFalse`, `CmdGetenv`, `CmdHostname`, `CmdPwd`, `CmdSet`, `CmdTrue`, `CmdUnalias`, `CmdUname`, `CmdUnset`, `CmdUptime`, `CmdVersion`, `CmdWhoami`, `CmdYield` |
...
| `kernel/shell/shell_process.cpp` | `CmdFree`, `CmdKill`, `CmdPs`, `CmdSpawn`, `CmdTop` |
| `kernel/shell/shell_security.cpp` | `CmdAttackSim`, `CmdCaplog`, `CmdGuard`, `CmdIdleLock`, `CmdLoginCmd`, `CmdLogout`, `CmdPasswd`, `CmdPolicy`, `CmdPurple`, `CmdSecEvents`, `CmdSu`, `CmdUnlock`, `CmdUseradd`, `CmdUserdel`, `CmdUsers` |
