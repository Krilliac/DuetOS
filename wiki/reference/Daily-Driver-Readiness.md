...
|-------|-------|---------------|-------------|
| **Disk installer — orchestration** | `install <handle> INSTALL` shell command lays down a 3-partition GPT (ESP / system / crash-dump), formats ESP and system as FAT32, seeds `/esp/boot/grub/grub.cfg` with a chainload stub, and mounts the new partitions at `/esp` + `/system`. Admin-gated; requires the literal `INSTALL` confirmation token. Source: `kernel/fs/installer.{h,cpp}`, `kernel/shell/shell_storage.cpp::CmdInstall`. | **Bootloader-bytes copy** — write a real `BOOTX64.EFI` and `duetos-kernel.elf` into the freshly-formatted ESP. Requires either embedding the running kernel image into the ramfs (a build-time bootstrap problem — kernel.elf bytes change after you embed them; classic two-stage solution) or pulling them from an out-of-band source (USB / network share / ISO chainload). v0 leaves a sentinel grub.cfg pointing at the expected on-disk paths so the layout is correct when the bytes arrive. | End-user features → Disk installer (shrunken to bootloader-bytes residual) |
