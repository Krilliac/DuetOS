...
|-------|-------|---------------|-------------|
| **Disk installer — orchestration + boot staging (not yet self-booting)** | `install <handle> INSTALL [--duetfs]` lays down a 3-partition GPT (ESP / system / crash-dump), formats the ESP as FAT32 and the system as FAT32 (default) or DuetFS (`--duetfs`), writes `/esp/boot/grub/grub.cfg`, stamps the **experimental** direct-loader `BOOTX64.EFI` at the UEFI removable-media filename, optionally stages an `.incbin`-embedded kernel ELF when `DUETOS_INSTALLER_KERNEL_EMBED=ON`, and mounts `/esp` + `/system`. Admin-gated; literal `INSTALL` confirmation token. Pure-math `PlanLayout` self-test runs every boot. Separately, `tools/image/build-removable-media.sh` now creates a regular-file-only GPT/FAT32 image with standalone GRUB at the UEFI removable path; `tools/test/removable-media-boot-smoke.sh` boots that artifact through OVMF to `bringup-complete`. This proves a safe host-built removable artifact, not an installed-disk path: the in-OS installer still does not install supported GRUB BIOS/EFI images, and its stamped direct loader halts after ELF-header validation. | Install the same verified GRUB artifacts from the in-OS installer so either BIOS or UEFI firmware reaches GRUB and GRUB performs the Multiboot2 handoff. Alternatively, the direct loader must first load ELF segments, call `ExitBootServices`, pass a versioned `BootInfo`, hand off to the kernel, and pass required CI. Add a reviewed Secure Boot signing/shim chain before claiming boot on machines that enforce Secure Boot. | In progress |
| **Writable native FS** | DuetFS shipped with the full write surface: `duetfs_write_at`, `duetfs_create_path`, `duetfs_unlink_path`, `duetfs_truncate`, `duetfs_link`, `duetfs_create_symlink`, journal (`duetfs_journal_apply` / `duetfs_journal_state`), CRC-checked blocks, AES-XTS sector encryption + Argon2id KDF, LZ4 compression, snapshots. RAM-backed `/duetfs` mount is online on every boot; on-disk `/disks/duetfsN` mounts auto-register. The disk installer's `--duetfs` flag formats the system partition as DuetFS instead of FAT32 — gives operators a journalled, encryption-capable system filesystem out of the box. FAT32 read+write (LFN both directions) and ramfs also writable. ext4 / NTFS read-only. | Boot the kernel from a DuetFS root rather than from a FAT32 ESP and then pivoting. That requires a DuetFS reader in the supported GRUB path, or a kernel loaded from the FAT32 ESP that mounts DuetFS as `/`. The optional installer kernel embed can stage the kernel today; the DuetFS boot/root handoff remains pending. | Filesystem → DuetFS follow-ups |
...
| Topic | Today | What's needed | Roadmap row |
|-------|-------|---------------|-------------|
| **Audio playback** | DMA-coherent BDL + PCM ring allocated, BDL filled, stream armed + RUN toggled by `subsystems/audio/audio_backend`; producer path live (`SYS_AUDIO_WRITE` 210 ← winmm `waveOutWrite`); QEMU smoke runs `-device intel-hda -device hda-output`; backend active + self-test PASS (silence/sine/Start/Stop/LPIB) every boot. | **Audible path now works on QEMU**: the CORB/RIRB stall is fixed by an Immediate-Command-Interface fallback (≈ Linux `single_cmd`; QEMU runs the CORB engine only once) and the codec self-test no longer wipes the shared jack inventory (snapshot/restore). Codec walk → DAC+line-out → `ConfigureOutputPath` → `codec routed` → `DMA LPIB advanced (routed, audible path)`, verified every boot. Remaining: real-HW audible validation (no HW in CI); multi-producer mixer. | Drivers → Audio; Track 12 → T12-03 |
...
| Topic | Today | What's needed | Roadmap row |
|-------|-------|---------------|-------------|
| **Structured Exception Handling** | **Landed (T6-02 slices 1-3).** Unwinder (`RtlCaptureContext` / `RtlLookupFunctionEntry` — now cross-module via `SYS_MODULE_BASE_BY_VA` / `RtlVirtualUnwind` / `RtlCaptureStackBackTrace`) plus the full kernel-fault → user dispatch: a ring-3 #DE/#OF/#BR/#UD/#GP/#PF/#MF/#AC/#XM in a Win32 PE is delivered as a Microsoft `EXCEPTION_RECORD`+`CONTEXT` (each mapped to its matching `STATUS_*` via the shared `VectorToUserNtStatus` table in `arch/traps`; #MF/#XM use a generic `STATUS_FLOAT_*` pending FSW/MXCSR decode) resumed at `ntdll!KiUserExceptionDispatcher`, which runs the Vectored Exception Handler chain then the frame-based `__C_specific_handler`/`RtlUnwindEx`/`RtlRestoreContext` walk; `NtContinue`/`NtRaiseException` real. Verified by `userland/apps/seh_pe` (VEH, `smoke=pe-hello`) **and** `userland/apps/seh_try_pe` — real MSVC `__try`/`__except`/`__finally` built with `clang --target=x86_64-pc-windows-msvc -fasync-exceptions` against our own `kernel32.lib`/`ntdll.lib`: null-write #PF, divide-by-zero #DE, `__finally`-on-unwind, and a repeatable case all PASS; zero browser regression. | C++ EH (T6-05) **engine landed, verification blocked**: `vcruntime140` now has a real FH3 personality (`__CxxFrameHandler3` — FuncInfo/try/catch decode, type match incl. `catch(...)`/base, destructor unwind, catch funclet) + `_CxxThrowException` (raises `0xE06D7363` into ntdll's `NtRaiseException`); links `ntdll.lib`; clang `windows-msvc` `cxxeh_pe` test wired into `smoke=pe-hello`. Builds clean (debug+release). Remaining: `cxxeh_pe` faults `0xC0000005`@`0x23d8` pre-test with loader `ntdll!NtRaiseException IAT not mapped` — a vcruntime140→ntdll import-resolution/preload-order issue (not the EH algorithm). Next: trace cross-DLL IAT binding for vcruntime140's ntdll imports. | Track 6 → C++ EH (T6-05) |
| **Win32 synchronization (V8/Chrome thread-pool)** | **Landed.** Address-keyed wait is real end-to-end: kernel `SYS_WAIT_ON_ADDRESS` / `SYS_WAKE_BY_ADDRESS` futex (address-hashed wait queues, spurious-but-never-lost wakeups) backs userland `WaitOnAddress` / `WakeByAddressSingle` / `WakeByAddressAll`, condition variables (`InitializeConditionVariable`, `SleepConditionVariableCS`/`SRW`, `WakeConditionVariable`, `WakeAllConditionVariable`, sequence-counter algorithm), and the explicit `InitOnceBeginInitialize` / `InitOnceComplete`. Modern APIs bind through a two-tier api-set host resolver: a static contract→host table (`kernel/loader/apiset_static.cpp`) is consulted first for the curated common surface (~70 contracts spanning core / crt / security / service); contracts the table doesn't yet cover fall through to the original "first preloaded export by name" heuristic. Boot log distinguishes `via-apiset-table` vs `via-apiset-heuristic` so new contracts are grep-able for promotion. Verified by `userland/apps/sync_smoke` (`smoke=pe-hello`): cross-thread CV producer/consumer, WaitOnAddress handshake, two-call InitOnce — all PASS; zero browser regression. | SRW shared still aliases exclusive (no reader/writer split). Cross-process futex (shared-section addresses across processes) is in-process only. | Win10 API breadth → synchronization follow-ups (SRW reader/writer; cross-process futex) |
...
| **Real KASLR** | KASLR enabled in flavour; the experimental direct UEFI loader does not yet pick a random kernel base. | Memory-map scan in the direct loader, random 2 MiB-aligned base in 64 MiB window, versioned `BootInfo` handoff. | Track 5 → T5-03 |
...
| **Crash-dump persistence on installed disk** | End-to-end and fail-closed. **NVMe + AHCI panic-write paths require `GptFindCrashDumpRegion` to find a bounds-checked, DuetOS-owned `kDuetCrashDumpTypeGuid` partition.** There is no tail-of-drive fallback; without that reservation the disk write is skipped while serial/debugcon output remains available. `lastdump` surfaces the owned on-disk LBA + byte count, and `DiskPersistSelfTest` SKIPs rather than writing when the installer has not created the partition. | Real-hardware verification: boot media with an installer-created crash-dump reservation, trigger one controlled panic, then confirm `lastdump` reads back the persisted bytes. | Storage → Crash-dump persistence |
...
| **DMA protection (IOMMU on by default)** | VT-d discovery→enable implemented but gated OFF; AMD-Vi parse-only. Bus-master DMA is constrained today only by the NVMe/AHCI staging-buffer discipline. | Validate VT-d enforce against the live storage/USB/NIC DMA paths, flip the default ON, implement AMD-Vi enable. On real hardware a bad descriptor can scribble firmware / another OS's memory without it. | Hardware safety → Enable the IOMMU by default |
| **Hardware-damage safety contract** | Audited 2026-06-06: storage ownership gates, read-only NTFS/ext4, no voltage/RAPL/thermal/firmware/NVRAM writes, ME/PSP fence. exFAT ownership gate landed. | Hold the line as risky controllers (GPU clock/fan, NIC EEPROM, UEFI NVRAM, secure-erase) land — each must ship its default-inert gate in the same slice. The contract + per-controller preconditions live in [`Hardware-Safety`](../security/Hardware-Safety.md). | Hardware safety → (all rows) |
...
| **Real-hardware bring-up** | Hardware execution remains unverified. A regular-file UEFI removable image now boots through GRUB to `bringup-complete` under QEMU/OVMF, which validates the artifact path but not a physical controller, firmware, display, input, or power stack. | Boot the artifact on at least one Intel-NUC-class machine and one AMD laptop, then add an ARM64 board post-port. Secure Boot remains a separate signing/shim gate. |
