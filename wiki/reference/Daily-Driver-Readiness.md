...
|-------|-------|---------------|-------------|
| **Disk installer — orchestration + boot staging (not yet self-booting)** | `install <handle> INSTALL [--duetfs]` lays down a 3-partition GPT (ESP / system / crash-dump), formats the ESP as FAT32 and the system as FAT32 (default) or DuetFS (`--duetfs`), writes `/esp/boot/grub/grub.cfg`, stamps the **experimental** direct-loader `BOOTX64.EFI` at the UEFI removable-media filename, optionally stages an `.incbin`-embedded kernel ELF when `DUETOS_INSTALLER_KERNEL_EMBED=ON`, and mounts `/esp` + `/system`. Admin-gated; literal `INSTALL` confirmation token. Pure-math `PlanLayout` self-test runs every boot. This proves layout and byte staging, not a supported boot: the installer does not install the GRUB BIOS/EFI boot images, and the stamped direct loader halts after ELF-header validation. Source: `kernel/fs/installer.{h,cpp}`, `kernel/shell/shell_storage.cpp::CmdInstall`, `tools/build/gen-kernel-blob.sh`. | For the supported release contract, install the GRUB boot artifacts so either BIOS or UEFI firmware reaches GRUB and GRUB performs the Multiboot2 handoff. Alternatively, the direct loader must first load ELF segments, call `ExitBootServices`, pass a versioned `BootInfo`, hand off to the kernel, and pass required CI. The high-address `.kernel_elf_blob` placement already avoids consuming the legacy DMA zone; that does not by itself make the disk bootable. | In progress |
