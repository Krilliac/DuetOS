...
| Topic | Today | What's needed | Roadmap row |
|-------|-------|---------------|-------------|
| **Structured Exception Handling** | **Unwinder landed (T6-02 slices 1-2).** `RtlCaptureContext` (real ms_abi register snapshot), `RtlLookupFunctionEntry` (real `.pdata` binary-search, main EXE), and `RtlVirtualUnwind` (real UNWIND_INFO unwind-code interpreter: PUSH_NONVOL / ALLOC_* / SET_FPREG / SAVE_NONVOL[_FAR] / PUSH_MACHFRAME / chained info) — all exported from both kernel32 and ntdll so the via-dll resolve binds them, not the no-op SEH thunk. `RtlCaptureStackBackTrace` walks via them (EXE frames). Verified by `userland/apps/seh_pe` (`smoke=pe-hello`): a 3-deep EXE call chain unwinds correctly. `__try`/`__except` over a CPU fault still doesn't dispatch. | `RtlUnwindEx`, `NtRaiseException`, cross-module `RtlLookupFunctionEntry` (loaded-DLL .pdata), and the kernel #PF/#GP → CONTEXT/EXCEPTION_RECORD → `KiUserExceptionDispatcher` user delivery walking the handler chain (`__C_specific_handler`) to the `__except`. | Track 6 → T6-02 (fault-dispatch slice) |
