...
|-------|-------|---------------|-------------|
| **Disk installer** | ISO live-boot only; no install path. The primitives ship — `fs::gpt::GptInitDisk`, `fs::fat32::Fat32Format`, `fs::gpt::FormatGuid` — but no orchestration. | Userland or shell installer that walks `pick-disk -> confirm-erase -> GPT-init -> FAT32-format -> copy kernel + initrd -> install bootloader to ESP`. | End-user features → Disk installer |
...
| **System updater** | None. | Code-signing infrastructure + A/B kernel-slot layout. | End-user features → System updater |
| **ACPI S5 / soft-off** | Shutdown calls `KernelHalt`: logs, masks IRQs, parks boot CPU. AML namespace walker drives QEMU's `\_S5_` path; consumer firmware that needs `_PTS` / `_GTS` method evaluation may stay powered. | AML method interpreter so `\_S5_`, `_PTS`, `_GTS` evaluate fully on real chipsets. | End-user features → ACPI S5 / soft-off |
