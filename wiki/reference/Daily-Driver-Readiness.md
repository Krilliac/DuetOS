...
| Topic | Today | What's needed | Roadmap row |
|-------|-------|---------------|-------------|
| **Static TLS (`__declspec(thread)`)** | **Landed (T6-01).** The PE loader parses `IMAGE_DIRECTORY_ENTRY_TLS` from the *mapped* (relocated) image, copies the `.tls` template (Start..End) + `SizeOfZeroFill` into a process TLS block, wires `TEB.ThreadLocalStoragePointer` (`gs:[0x58]`) → slot array → block, writes `*_tls_index`, and invokes every registered TLS callback before entry via a generated R-X trampoline (Win64 ABI, `DLL_PROCESS_ATTACH`). The old `TlsCallbacksUnsupported` hard-reject is gone — the universal gate for loading real MSVC/clang Windows binaries. Verified by `userland/apps/tls_pe` under `smoke=pe-hello`. **Remaining:** the block is per-process (main-thread); per-*thread* static-TLS replication + `DLL_THREAD_ATTACH` on `SYS_THREAD_CREATE` is the follow-on for multi-threaded workloads (Chrome). | Per-thread TLS block allocation + `DLL_THREAD_ATTACH`/`DLL_THREAD_DETACH` callback invocation wired into thread create/exit. | Track 6 → T6-01 (per-thread half) |
