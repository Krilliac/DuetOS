...
| Topic | Today | What's needed | Roadmap row |
|-------|-------|---------------|-------------|
| **Structured Exception Handling** | **Foundation landed (T6-02 slice 1).** `RtlCaptureContext` is a real register snapshot (naked ms_abi, exported from both kernel32 and ntdll so the via-dll resolve binds it instead of the no-op SEH thunk); `RtlLookupFunctionEntry` is a real table-based `.pdata` (IMAGE_DIRECTORY_ENTRY_EXCEPTION) binary-search over the main EXE. Verified by `userland/apps/seh_pe` (`smoke=pe-hello`). `__try`/`__except` over a CPU fault still doesn't unwind. | `RtlVirtualUnwind` unwind-code interpreter, `RtlUnwindEx`, `NtRaiseException`, and the kernel #PF/#GP → `KiUserExceptionDispatcher` user delivery that walks the handler chain to the `__except`. | Track 6 → T6-02 (fault-dispatch slice) |
