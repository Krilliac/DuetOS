...
| T6-02 | kernel | P1 | x64 SEH. **Slice 1 landed:** real `RtlCaptureContext` (naked ms_abi register snapshot, in kernel32_interlocked.c + ntdll_seh.c, exported from both so the via-dll resolve binds it not the no-op thunk) + real table-based `RtlLookupFunctionEntry` (.pdata binary search over the main EXE). Verified by `userland/apps/seh_pe`. **Remaining:** `RtlVirtualUnwind` unwind-code interpreter (RBP/alloc/push-nonvol/save-xmm), `RtlUnwindEx`, `NtRaiseException`, and the kernel #PF/#GP → CONTEXT/EXCEPTION_RECORD → `KiUserExceptionDispatcher` user delivery walking the handler chain (`__C_specific_handler`) to the `__except`. | A PE `__try`/`__except` null write is caught and continues in the exception handler. |
