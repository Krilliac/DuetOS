...
| ID | Scope | Priority | Task | Acceptance |
| --- | --- | --- | --- | --- |
| T6-01 | kernel | P1 | **Mostly landed.** Dynamic TLS (`TlsAlloc`/`TlsSetValue`/`TlsGetValue`/`TlsFree`, per-thread) shipped earlier. Static TLS + TLS callbacks landed in `kernel/loader/pe_loader.cpp::SetupStaticTls`: TLS directory parsed from the mapped (relocated) image, `.tls` template + zero-fill copied into a process TLS block, `TEB.ThreadLocalStoragePointer` + `*_tls_index` wired, callbacks invoked before entry via a generated R-X trampoline. The `TlsCallbacksUnsupported` reject is removed. Verified by `userland/apps/tls_pe` (`smoke=pe-hello`). **Remaining (this row):** the static-TLS block is per-process/main-thread only — replicate it per-thread on `SYS_THREAD_CREATE` and fire `DLL_THREAD_ATTACH`/`DLL_THREAD_DETACH` so multi-threaded `__declspec(thread)` workloads (Chrome) get independent per-thread storage. | A PE with `__declspec(thread) int x = 42` reads independent values from two concurrently-running threads. |
