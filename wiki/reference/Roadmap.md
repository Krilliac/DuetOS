...
| T6-02 | kernel | P1 | x64 SEH. **Slices 1-2 landed:** real `RtlCaptureContext` (naked ms_abi snapshot) + `RtlLookupFunctionEntry` (.pdata binary search, main EXE) + `RtlVirtualUnwind` (full UNWIND_INFO unwind-code interpreter: PUSH_NONVOL, ALLOC_SMALL/LARGE, SET_FPREG, SAVE_NONVOL[_FAR], SAVE_XMM skip, PUSH_MACHFRAME, chained info) + `RtlCaptureStackBackTrace` — all in kernel32_interlocked.c + ntdll_seh.c, exported from both. Verified by `userland/apps/seh_pe`: a 3-deep EXE call chain unwinds correctly. **Remaining:** `RtlUnwindEx` (transfer control to a target frame), `NtRaiseException`, cross-module `RtlLookupFunctionEntry` (resolve loaded-DLL .pdata, not just the EXE — needed so backtrace can cross the kernel32/ntdll boundary), and the kernel #PF/#GP → CONTEXT/EXCEPTION_RECORD → `KiUserExceptionDispatcher` user delivery walking `__C_specific_handler` to the `__except`. | A PE `__try`/`__except` null write is caught and continues in the exception handler. |
