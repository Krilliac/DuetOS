...
>
> `CreateThread`, `ExitThread`, `FreeLibraryAndExitThread`, and
> `GetExitCodeThread` are the first fail-closed retirement wave:
> x64 callers must bind their verified user-mode DLL exports and
> cannot fall back to legacy kernel thunk rows.
...
**`kernel32.dll`** — 409 imports thunked: 409 REAL.
...
**`msvcrt.dll`** — 55 imports thunked: 55 REAL.
...
**`vcruntime140.dll`** — 25 imports thunked: 25 REAL.

| Method | Status | Routed to |
|--------|--------|-----------|
| `__C_specific_handler` | REAL | `kOffTerminate` |
...
**`ucrtbase.dll`** — 61 imports thunked: 61 REAL.
