...
- Exception dispatch (2026-07-28): `RaiseException` is a real
  export — `raise_exception.S` captures the caller's `CONTEXT`
  before any prologue runs and `kernel32_seh.c` builds the
  `EXCEPTION_RECORD` and enters `ntdll!NtRaiseException`, the same
  two-pass engine a CPU fault uses. It previously routed to
  `SYS_EXIT(dwExceptionCode)`, which is why a statically-linked
  MSVC CRT's `throw` surfaced as a process exit with status
  `0xE06D7363` rather than an unwind. `RtlUnwindEx` / `RtlUnwind` /
  `RtlRestoreContext` are PE forwarders to `ntdll` — one unwinder,
  reached from either import name. `RtlCaptureContext` /
  `RtlLookupFunctionEntry` / `RtlVirtualUnwind` /
  `RtlCaptureStackBackTrace` keep kernel32-local bodies (they are
  pure and must stay byte-identical with ntdll's — audit both on
  any change).
...
- Lifecycle: `_initterm`, `_initterm_e`,
  `__C_specific_handler` (returns ExceptionContinueSearch),
...
- **STUB** — the C++ EH surface. `__CxxFrameHandler3` /
  `__CxxFrameHandler4` / `__C_specific_handler` return
  `ExceptionContinueSearch`, so no `__except` filter and no `catch`
  block ever runs; `_CxxThrowException` terminates instead of
  unwinding. The `exception` constructors leave the object
...
**`msvcrt.dll`** — 78 imports thunked: 78 REAL.
...
| `__CxxFrameHandler3` | REAL | `kOffPinReturn1` |
| `__CxxFrameHandler4` | REAL | `kOffPinReturn1` |
...
**`vcruntime140.dll`** — 24 imports thunked: 24 REAL.
