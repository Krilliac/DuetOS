...
| **CPU frequency telemetry** | Read-only | `arch/x86_64/cpufreq.cpp` reads current/base/min + effective (MPERF/APERF) frequency. Probed per-MSR through `ReadMsrSafe`, not predicted from vendor + hypervisor |
| **CPU P-state control** | **Default-inert, operator-unlocked (landed 2026-07-29)** | `arch/x86_64/cpufreq_ctl.cpp` — the only file in the tree that writes a CPU operating-point MSR, split out of `cpufreq.cpp` (which stays read-only, row above) precisely so this is answerable by file name. It MAY write `IA32_PERF_CTL` / `IA32_HWP_REQUEST` (Intel) or `MSR_PSTATE_CTL` (AMD) — **ratio / P-state index selection only, never voltage**. Owner-approved 2026-07-29; superseded the previous "never writes `IA32_PERF_CTL`/HWP" row. Three gates, all required: (1) `cpufreq=tune` on the boot cmdline — off by default; (2) `kCapPowerTune` on the caller; (3) the value must fall inside a window read this boot from the part's own registers (`IA32_PLATFORM_INFO` + `MSR_TURBO_RATIO_LIMIT`, or `IA32_HWP_CAPABILITIES`, or the enabled `MSR_PSTATE_DEF` entries) — out-of-window is **refused, not clamped**. All writes go through `WriteMsrSafe` (extable-recovered #GP) and are read back and verified. **No syscall exposes it** — no guest PE/ELF can drive the clock, which is both a thermal and a side-channel (Hertzbleed) containment. Still forbidden: `MSR_PSTATE_DEF` (couples `CpuVid`), the OC mailbox `0x150`, RAPL limits, PROCHOT/thermal-throttle defeat, and enabling HWP where firmware left it off. See [Power-Management](../drivers/Power-Management.md) |
...
| **MSR voltage / Vcore offset** (`0x150`, OC mailbox) | PHYS-DMG | Undervolt faults silicon (Plundervolt); overvolt degrades the die | **Still forbidden outright — no code path writes a voltage.** Note the 2026-07-29 P-state slice did NOT open this: it selects a ratio (Intel) or an index into the platform's own pre-validated P-state table (AMD), so no `(ratio, voltage)` pair is ever synthesised, and `MSR_PSTATE_DEF` — whose `CpuVid` field would couple one — is never written |
