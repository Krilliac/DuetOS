...
- [ ] A1-followup — Continue `kernel_main` call-site migration to `RunPhase(...)` (Earlycon + PhysMem + Heap + Paging + Idt + Apic + Time migrated; Sched / Drivers / Vfs / Userland remain)
...
- [ ] A2-followup — Migrate existing `arch::TimerTicks()` call sites to `time::TickCount()` so the arch backend can be swapped in (the wrapper landed; consumers haven't been migrated)
