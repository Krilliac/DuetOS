...
- [ ] A1-followup — Continue `kernel_main` call-site migration to `RunPhase(...)` (Earlycon utility-self-tests already done; PhysMem / Paging / Heap / Idt / Apic etc. remain)
...
- [ ] A3-followup — Concrete `KMailbox` / `KWaitable` subclasses (KMutex / KEvent / KSemaphore landed; KMailbox needs a bounded queue + producer/consumer condvar pair; KWaitable is the multi-object WaitForMultipleObjects superset)
