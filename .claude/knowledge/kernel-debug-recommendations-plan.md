...
- [ ] A1-followup — Continue `kernel_main` call-site migration to `RunPhase(...)` (Earlycon + PhysMem + Heap + Paging + Idt + Apic + Time + Sched migrated; Drivers / Vfs / Userland remain)
...
- [ ] A2-followup — Continue migrating remaining `arch::TimerTicks()` call sites to `time::TickCount()` (klog / random / runtime_checker / net done; ABI-shaped sites — Win32 mutex_syscall, Linux syscall_time, gfxdemo, custom.cpp, time_syscall, panic, shell_hardware — remain)
