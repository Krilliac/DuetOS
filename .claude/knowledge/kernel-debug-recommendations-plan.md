...
- [ ] A1-followup — Final `kernel_main` call-site migration to `RunPhase(...)`: only `Phase::Userland` remains (every other phase has been migrated)
...
- [ ] A2-followup — Final `arch::TimerTicks()` call-site migrations: 3 sites remain (`kernel/syscall/time_syscall.cpp`, `kernel/shell/shell_hardware.cpp`, `kernel/subsystems/win32/mutex_syscall.cpp`); klog / random / runtime_checker / net / panic / gfxdemo / syscall_time / custom done
