...
- [ ] A3-followup — Concrete `KEvent` / `KSemaphore` / `KMailbox` / `KWaitable` subclasses (KMutex landed; the `kobj_handles` table on `Process` is in place; next slice repeats the KMutex shape for the rest)
