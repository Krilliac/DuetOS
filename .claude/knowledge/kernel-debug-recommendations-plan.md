...
| _D7-followup_ (this commit) | GDB stub `g` register-read packet now serves a live snapshot. New `GdbRegSnapshot` struct (16 GPRs + RIP + RFLAGS + 6 segment regs in GDB's canonical x86_64 order) + `GdbStubPublishRegisters(snap)` accessor. `g` handler hex-encodes the snapshot little-endian; nullptr = zero-fill (existing behaviour). Trap-frame → snapshot wiring is the next slice. |
| _D7-followup_ (this commit) | GDB stub `m<addr>,<len>` packet now reads real kernel-VA bytes. Parser handles hex addr + comma + hex len; bounds the reply at half the packet capacity; rejects non-canonical addresses with `E14` (EFAULT-style). Read is a direct dereference today — extable-protected access stays on the deferred list since a fault during a debug read should fall through to the existing #PF handler, which already has a recovery path through extable. |
| _D2 instrumentation_ (this commit) | Event tracer wired into two real call sites: `kEventSyscallEnter` (kind, syscall_nr, frame->rdi) emitted from `SyscallDispatch`'s entry; `kEventMutexAcquire` / `kEventMutexRelease` (mutex_addr, current_tid) emitted from `sched::MutexLock` / `sched::MutexUnlock` after the lockdep hooks. `tracer dump` now shows real boot traffic instead of the synthetic self-test stream. Exit hook on the syscall path skipped for now — dispatch has many early-returns and the Enter event alone is enough to correlate the in-flight stream. |
| _shell_ (this commit) | `tracer reset` verb — admin-gated; calls `EventTraceReset()` to wipe every CPU's ring + total counter before kicking off a measurement. Pairs naturally with `tracer dump` / `tracer kind <name>`. |
| _E3-followup_ (this commit) | Fifth driver registered as a fault domain: `nmi-watchdog`. Init = `arch::NmiWatchdogInit` (re-probes CPUID + re-arms PMU MSRs); teardown = `arch::NmiWatchdogDisable`. `domain restart nmi-watchdog` cycles the watchdog so an operator can quiet it for a long-running diagnostic window then re-arm. Brings the registered driver-domain set to 5 (soft-lockup + lockdep + event-trace + perf + nmi-watchdog). |
...
| _D7-followup_ (this commit) | Trap-dispatch panic path also publishes the same `s_gdb_snap` as a WRITABLE snapshot via `GdbStubPublishWritableRegisters`. A connected GDB session's `G` packet now applies edits in-place. Resume-after-fault wiring (copying the edited snapshot back into the trap frame before `iretq`) is the remaining D7-followup once a real stop/resume flow exists. |
