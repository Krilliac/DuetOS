...
- [ ] B1.3 — `kernel/sync/seqlock.{h,cpp}` — for read-mostly hot data (timekeeper, per-CPU counters)
- [ ] B1.4 — `kernel/sync/rcu.{h,cpp}` — quiescent-state RCU keyed off the scheduler tick
- [ ] B1-followup — Migrate `AddressSpace`'s rolled-own RW lock to `sync::RwLock`
...
- [ ] A2-followup — Migrate `DoNowNs` / `SYS_NOW_NS` to `time::MonotonicNs()` (one-line forward)
- [ ] A2-followup — Add TSC clocksource (rating 300) once invariant-TSC calibration is implemented
- [ ] A2-followup — Add `time::RealtimeFiletime()` / `BoottimeNs()` and migrate `DoGetTimeFt` / `DoGetTimeSt`
- [ ] A2-followup — Promote scheduler tick + periodic timer ownership into `kernel/time/timer.cpp` and `kernel/time/tick.cpp`
- [ ] D1-followup — Add `class_id` field to `SpinLock` / `Mutex` / `RwLock`; tag the hot global locks (frame_allocator, kheap, sched runqueue) and call `LockdepBefore/After*` in their acquire/release paths
