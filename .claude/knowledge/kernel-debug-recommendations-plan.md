...
- [ ] D7-followup — Implement `G`/`M` (write registers + write memory) — currently returns OK without applying writes
- [ ] D7-followup — Wrap the `m`-handler memory read in extable-protected access so a bad address from GDB doesn't fault the kernel (today it relies on the `#PF` handler's existing recovery)
