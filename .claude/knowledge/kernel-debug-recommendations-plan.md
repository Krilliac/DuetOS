...
- [ ] A1-followup — Continue `kernel_main` call-site migration to `RunPhase(...)` (Earlycon + PhysMem + Heap migrated; Paging / Idt / Apic / Time / Sched / Drivers / Vfs / Userland remain)
...
- [ ] B1-followup — SMP-stress versions of the RwLock + SeqLock contention self-tests (current cooperative-single-CPU forms verify the wakeup paths fire; AP bringup will let real concurrent acquires race on the spinlock cores)
- [ ] B1-followup — Migrate the timekeeper to `sync::SeqLock` once it owns wall-clock state worth racing against
- [ ] A3-followup — Concrete `KWaitable` subclass (KMutex / KEvent / KSemaphore / KMailbox landed; KWaitable is the multi-object WaitForMultipleObjects superset — needs a wait-set abstraction over the four primitives' is-signaled predicates)
...
- [ ] D1-followup — Tag remaining per-instance `sched::Mutex` and `sync::RwLock` users (audio_server, IPC handle table, network stack, …); compositor + fat32 done; current named-class roster is 9 (sched / kobject / kstack / pci-config / breakpoints / cleanroom-trace / wifi / fat32 / compositor)
