...
- [ ] D2-followup — Add a `tracer dump` shell command + `tracer kind <K>` filter knob (the API + storage are landed; the operator-facing surface lands when an investigation needs it)
