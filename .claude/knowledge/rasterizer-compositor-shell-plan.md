...
| Slice | Effect | Commit |
|-------|--------|--------|
| Plan landed | This file + index row | _this commit_ |

Deferred (in execution order):

1. Slice 1 — shadow framebuffer foundation (offscreen u32 buffer
   + `FramebufferBeginCompose` / `EndCompose` + retarget the 5
   primitive write sites; `DesktopCompose` paints into shadow,
   memcpys shadow → live, then calls `FramebufferPresent`).
2. Slice 2 — true alpha-blend onto shadow surface (read-modify-write
...
3. Slice 3 — TTF table parser (head / maxp / hhea / cmap / loca /
...
4. Slice 4 — TTF scanline rasterizer (winding-rule fill of glyf
...
5. Slice 5 — userland crt0 + minimal libc + CMake ELF build rule.
...
6. Slice 6 — prompt-driven shell (read line via SYS_READ on the
   serial console, dispatch a tiny built-in command table:
   `help`, `pid`, `echo`, `exit`).
7. Slice 7 — SVG loader (subset: `<svg>`, `<path d=...>`,
