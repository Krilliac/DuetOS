...
| Per-PR smoke testing for every flavor | **Landed.** `build-flavor-matrix` in build.yml builds + boots + asserts signatures for each flavor on every PR push. `flavor-smoke-gate` in release.yml does the same, opt-in via the `RELEASE_SMOKE_GATE` workflow var (same trigger as the existing `release-smoke-gate`); when on, a failed flavor smoke blocks `publish-flavor-channels` so a broken flavor never ships. Both use the **`bringup` profile** (kernel reaches bringup-complete + sentinel + isa-debug-exit; ~30-60s of guest time per attempt under TCG with 480s timeout) rather than the full ctest 33-signature smoke — sufficient signal to validate compile-time flavor axes (cap-audit, lockdep, asserts, klog floor, ubsan) up through driver init, without the cost of a full ring3 + Win32 + driver matrix that GitHub-hosted runners (no KVM) couldn't fit in budget. Both `ctest-boot-smoke.sh` AND `profile-boot-smoke.sh` auto-detect from the boot banner whether `kBootSelfTests` was on, so selftest-pass-marker signatures are required only when the banner shows `+selftests` — lets the same drivers run uniformly across debug, release, and every flavor. |
| KNOWN GAP — debug-fast VFS-phase panic | **Open.** The `x86_64-debug-fast` preset panics at `RunPhase(Vfs)` under the full ctest-boot-smoke driver. Doesn't reproduce under the `bringup` profile because bringup exits before Vfs phase. Since `DUETOS_LOCK_ORDER_AUDIT` is the only meaningful preset delta from `x86_64-debug` and that flag only gates the boot banner (no runtime-code branches today), debug-fast SHOULD be byte-identical to debug at runtime. The panic is therefore likely a flake or a latent VfsSelfTest race that just hasn't been seen on debug yet. Tracked as a follow-up; bringup smoke catches the flavor axes we care about for the matrix. |
