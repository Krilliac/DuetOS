...
| Per-PR smoke testing for every flavor | **Landed.** `build-flavor-matrix` in build.yml now builds + boots + asserts signatures for each flavor on every PR push. `flavor-smoke-gate` in release.yml does the same, opt-in via the `RELEASE_SMOKE_GATE` workflow var (same trigger as the existing `release-smoke-gate`); when on, a failed flavor smoke blocks `publish-flavor-channels` so a broken flavor never ships. The smoke driver auto-detects from the boot banner whether `kBootSelfTests` was on; selftest-pass-marker signatures are required only when the banner shows `+selftests`. Lets the same driver run uniformly across debug, release, and every flavor. |
