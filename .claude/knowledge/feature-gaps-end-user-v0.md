...
### 5. Settings panel
- **Today:** theme switching, opacity, display-cycle, login mode
  are all keyboard-only Ctrl+Alt chords. Nine themes are hardcoded
  in `kernel/drivers/video/theme.cpp`.
- **Expected:** a Settings window listing Display, Sound, Network,
  Time, Keyboard, About — clickable.
- **Owners:** new `kernel/apps/settings.{h,cpp}` (does not exist).
