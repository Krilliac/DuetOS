...
| P0 #4 Wi-Fi connect-to-SSID | Real-hardware verification cycles + ~~DMA-coherent allocation API (`mm::AllocDmaCoherent` does not exist)~~ (landed 2026-05-03 — `dma-coherent-v0.md`; iwlwifi TFD/RBD rings are the first consumer, ring buffers now allocated in Dma32 zone with FH base regs programmed with real phys addrs) + ~~AES key wrap (RFC 3394) for encrypted M3 key data~~ (primitives + EAPOL M3 KeyData unwrap landed 2026-05-03 — `aes-and-keywrap-v0.md` + `crc32-md5-base64-and-eapol-keywrap-v0.md`) + IRQ wiring on per-vendor MSI/MSI-X + iwlwifi TFD descriptor build / doorbell / per-RBD data buffers (next slice for the rings consumer). Data-decode tier (envelope parsers + beacon walker) AND control tier (crypto + EAPOL + 4-way handshake + wdev/MLME + per-vendor upload + ring scaffolds) all landed 2026-05-01 | Network flyout SSID picker, Settings → Network → Wi-Fi tab, captive-portal handler |
