...
- ~~**Real rlimit enforcement**~~ — landed for NOFILE + NPROC: per-Process soft caps (`linux_rlimit_nofile_cur`, `linux_rlimit_nproc_cur`) initialised to 0xFF... sentinel ("no cap below kernel ceiling"); setrlimit/prlimit64 lower them; getrlimit reports them. NOFILE enforced in DoOpen / DoDup / F_DUPFD via `LinuxFdEffectiveMax(p)`; NPROC enforced in DoFork via `SchedCountChildrenOfPid` (live-child count vs. cap, returns -EAGAIN like Linux). Sub-GAPs: auxiliary fd allocators (signalfd, timerfd, eventfd, inotify, fanotify, pipe, msgq, memfd, pidfd) still hardcode 16 — not yet routed through LinuxFdEffectiveMax; STACK / AS / DATA still constants. CLONE_THREAD doesn't burn NPROC (same Process)
