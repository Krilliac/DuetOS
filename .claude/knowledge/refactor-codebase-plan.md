...
|---|---|---:|---:|---|
| 1 | `kernel/core/shell.cpp`               | 9,769 | 8,581 | ◐ in progress (5 command slices + 5 helper hoists + utility-bucket-1; ~12.2% trim. Env-table + alias-table + history-ring → shell_state.cpp; path/parse helpers → shell_pathutil.cpp; output formatters → shell_format.cpp; StrEq / StrStartsWith / kInputMax / kHistoryCap inline in shell_internal.h. CmdBasename / CmdDirname / CmdFlushTlb / CmdMem / CmdMode / CmdHistory / CmdSleep / CmdShutdownNow → shell_utilities.cpp) |
