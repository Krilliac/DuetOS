...
|---|---|---:|---:|---|
| 1 | `kernel/core/shell.cpp`               | 9,769 | 5,509 | ◐ in progress (~43.6% trim from peak). Hoisted helpers: env / alias / history rings (shell_state.cpp), path + parse + I/O helpers (shell_pathutil.cpp + shell_fsio.cpp), output formatters (shell_format.cpp), StrEq + IPv4 helpers (shell_internal.h). Extracted command buckets: shell_utilities.cpp (21 cmds), shell_filesystem.cpp (14 cmds), shell_hardware.cpp (22 cmds), shell_network.cpp (8 cmds), shell_core.cpp (env+alias+misc). Remaining in shell.cpp: dispatch table, Source / Repeat / Time, debug bucket (Bp/Probe/Inspect/DumpState/MemDump/Trace/Addr2Sym/Instr), heavier net (Dhcp/Route/Netscan/Wifi/FwPolicy/FwTrace/CrTrace/Net/UsbNet), Fat* family, security (Login/Logout/Su etc.) + admin gating + dispatch + input edit. |
