...
6. **Implement cross-process VM access** (NtOpenProcess, NtReadVirtualMemory, NtWriteVirtualMemory, NtQueryVirtualMemory) — **PARTIAL**: `NtOpenProcess` landed end-to-end (sched PID lookup, per-Process handle table at base 0x700, `SYS_PROCESS_OPEN = 131`, cap-gated on `kCapDebug`, refcount-held targets, ntdll thunk + Zw alias, NT-table mapping). NtReadVirtualMemory / NtWriteVirtualMemory / NtQueryVirtualMemory are the remaining piece — each needs a cross-AS PML4 walk to translate a target VA to a PhysAddr that the kernel can then access via `mm::PhysToVirt` + the direct map. ~500 LOC remaining. Unlocks the entire "real Windows malware" test surface.
