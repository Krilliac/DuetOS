...
7. **Implement thread manipulation** (NtSuspendThread, NtResumeThread, NtSetContextThread, NtGetContextThread) — **PARTIAL**: NtSuspendThread + NtResumeThread + NtAlertResumeThread landed end-to-end. `Task::suspend_count` (per-task, mutated under arch::Cli + g_sched_lock); `g_suspended_head/tail` intrusive list (Task::next, mutually exclusive with runqueue / wait queue / sleep queue / zombies); `RunqueuePopRunnable` re-parks suspended-while-Ready tasks lazily; `RunqueueOrSuspendPush` is the wake-path counterpart that routes by suspend_count. `SchedSuspendTask` + `SchedResumeTask` cross-task control APIs (modeled on `SchedKillByPid`); `SYS_THREAD_SUSPEND = 135` + `SYS_THREAD_RESUME = 136`; ntdll thunks NtSuspendThread / NtResumeThread / NtAlertResumeThread + Zw aliases; build script flips them from NotImpl to real; KNOWN_MAPPINGS gains 3 entries; NT-table mapping climbs 30→33. **GAPs**: (a) only caller-local thread handles (kWin32ThreadBase + idx in calling Process's win32_threads) — cross-process thread suspend needs NtOpenThread + a foreign thread handle table, which is its own slice. (b) NtSetContextThread / NtGetContextThread deferred — the Win32 CONTEXT struct is 1232 bytes, populating it correctly needs to walk into the suspended task's saved trap frame on the kernel stack, and not every suspended task has a trap frame at a known offset (kernel-thread context-switch saves only callee-saved regs). Designing that introspection is its own slice. (c) Killing a suspended task surfaces as KillResult::Blocked — the kill path doesn't auto-resume; resuming first then killing works. (d) On SMP, suspending a target running on another core needs an IPI; v0 single-CPU correctness covers all current paths. NtSuspendThread + NtResumeThread alone unlock the freeze-half of "thread hijack" tests.
