...
| Kernel breakpoint subsystem v0 + phase 2a (per-task DR, syscall, kCapDebug) + phase 3 (suspend/inspect/resume/step) + phase 4 (static KBP_PROBE macros, 16 sites: panic.enter, sandbox.denial, win32.stub_miss, mm.kernel_pagefault, trap.kernel_gpf, trap.kernel_ud, mm.heap_alloc_fail, mm.phys_alloc_fail, smp.ap_online, ring3.spawn, proc.create, proc.destroy, loader.pe_load, loader.elf_load, sched.thread_exit, sched.context_switch) | [knowledge/breakpoints-v0.md](knowledge/breakpoints-v0.md) | Observation | Active | 2026-05-04 |
...
| Crash dump v0 — embedded symbol table + bracketed text dump on COM1 + register-bit decoders + GPR symbolization + tree-wide hex log readability pass + VA-region tags + boot-time mm-map anchor + peer-CPU NMI snapshots + held-locks dump + return-address-pointer scan + page-walk for cr2/rip + Architectural LBR snapshot + per-task syscall trail + per-process VM info + loaded-DLL table; **and a structurally-valid Windows minidump (`.dmp`, MDMP-magic, CONTEXT_X64 + ThreadList + ModuleList + MemoryList + ExceptionStream + SystemInfo) emitted byte-by-byte over QEMU's debugcon (port 0xE9 → `${BUILD_DIR}/duetos.dmp` host file). Loadable in Visual Studio / WinDbg / VSCode-cppvsdbg / Python `minidump` lib / Mozilla `minidump-stackwalk` (validated end-to-end against the trap-demo path); real-hardware persistence (raw-block write to a reserved LBA range) deferred to v1** | [knowledge/crash-dump-v0.md](knowledge/crash-dump-v0.md) | Observation | Active | 2026-05-04 |
| **Live debug via in-kernel GDB server v0 — feature-complete** — RSP parser + qSupported / qXfer:features:read (24-reg target.xml) / g/G/m/M/H/c/s/Z0..Z4/z0..z4/D/k/vCont/vCont? + qfThreadInfo/qsThreadInfo/qC/T packets; trap-frame plumbing (#BP rolls RIP back, #DB clears TF) so step / break / continue / read+write registers + memory all work end-to-end. **SW + HW breakpoints backed by `kernel/debug/breakpoints` subsystem**; install errors translate `BpError` → distinct GDB E1n codes (E12 = NoHwSlot for the 5th-watchpoint case). **Three transports**: TCP (default), QEMU pty (software null-modem), real `/dev/ttyUSB*` (USB-UART cable to iron) — all driven by the same `duetos-gdb-attach.sh` script (`--via-pty` / `--com <device>`). AI-friendly batched helper: `duetos-gdb-cmd.sh`. **VSCode one-click attach** via `.vscode/launch.json` + `tools/debug/vscode-{start,stop}-qemu.sh`. Build flags: `DUETOS_GDB_SERVER` (default-ON in x86_64-debug) + `DUETOS_GDB_DEMO`. Env: `DUETOS_GDB_TRANSPORT=tcp\|pty\|stdio` selects the QEMU COM2 backend. **Async Ctrl-C stop, SMP NMI rendezvous, multi-thread visibility (peers as GDB threads with read+write registers via `Hg`+`G`+commit-on-release), `vCont` resume verb, and software null-modem all landed (2026-05-04).** Only deferred item is `vCont;s` step on a peer thread (no clear use case). | [knowledge/gdb-server-live-debug-v0.md](knowledge/gdb-server-live-debug-v0.md) | Observation + Pattern | Active — feature-complete | 2026-05-04 |
...
| Kernel attacker simulation suite v1 — **20 active attacks** (bootkit, IDT, GDT, LSTAR, SYSENTER_CS/EIP, CR0.WP, SMEP, SMAP, NXE, .text patch, ransom-burst, ransom-low-and-slow, canary, persistence, stack-canary defang, function-branch NOP, function-pointer table overwrite, saved-RIP smash, PTE W^X flip) + deferred catalogue | [knowledge/attack-sim-kernel-v1.md](knowledge/attack-sim-kernel-v1.md) | Observation + Pattern | Active | 2026-05-03 |
| Function-branch NOP-patch attack v0 + full-`.text` hash gap-fix — synthetic cap-gate inline-asm target, 2-byte `je`→`90 90` patch, BYPASS LANDED log line; `runtime_checker` gained `g_baseline_text_full_hash` + `ComputeTextFullHash` so middle-`.text` modifications now fire `KernelTextModified` (drift class logged: SPOT-only / FULL-only / SPOT+FULL); host unit test `tests/host/test_text_hash.cpp` proves the algorithm in 10 ms | [knowledge/branch-nop-attack-v0.md](knowledge/branch-nop-attack-v0.md) | Observation + Decision + Pattern | Active — gap closed in same slice; page-CRC variant deferred to a future slice if `.text` grows past ~10 MiB | 2026-05-03 |
| Slow boot under TCG — root cause + fix — recent commits added 100k-iteration PBKDF2 to the auth seed + an 8-attempt brute-force probe at boot; under TCG without SIMD that's ~30-60 wall-s of unhelpful work. Skip the brute-force probe on emulator + drop seed iterations to 1k on emulator. 4× boot-time speedup (kernel-time 82s→21s) | [knowledge/attack-sim-tcg-slowness-v0.md](knowledge/attack-sim-tcg-slowness-v0.md) | Issue + Pattern + Decision | Active | 2026-05-03 |
...
| **Account system v1** — per-account metadata (created_ns / last_login_ns / last_attempt_ns / failed_attempts / total_logins / locked_until_ns) + brute-force lockout (5 consecutive failures → 60 s freeze, auto-thaw on entry, locked-account verifies still burn a PBKDF2 cycle against decoy to flatten timing) + auth event publication on `event_ring` (7 new EventKind: AuthLoginSuccess/Failure/AccountLocked/Unlocked/Created/Deleted/PasswordChanged) + IR runbook entries for the 2 attack-signal events + admin `unlock` shell command + `users` shows lock + counts. Boot self-test drives the full lockout state machine end-to-end. Build clean both flavours; QEMU boot self-test passes (auth/secevents/ir all PASS) | [knowledge/account-system-v1.md](knowledge/account-system-v1.md) | Decision + Pattern + Observation | Active — persistence still pending (next bounded slice) | 2026-05-03 |
