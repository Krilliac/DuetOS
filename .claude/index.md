...
| **Live debug via in-kernel GDB server v0** — RSP parser + qSupported / qXfer:features:read (24-reg target.xml) / g/G/m/M/H/c/s/Z0..Z4/z0..z4/D/k packets; COM2 (0x2F8) wired via QEMU `-serial tcp::1234,server=on,wait=off`; trap-frame plumbing (#BP rolls RIP back, #DB clears TF) so step / break / continue / read+write registers + memory all work end-to-end. **SW + HW breakpoints backed by `kernel/debug/breakpoints` subsystem** (one source of truth; GDB-set BPs and shell `bp` BPs coexist). AI-friendly helpers: `tools/debug/duetos-gdb-attach.sh` (interactive) + `duetos-gdb-cmd.sh` (batched non-interactive). Build flags: `DUETOS_GDB_SERVER` (wire COM2 — default-ON in x86_64-debug preset) and `DUETOS_GDB_DEMO` (deliberate int3 for testing the attach plumbing without staging a real crash). **Async Ctrl-C stop landed (2026-05-04)** — `gdb::PollAsyncStop` hooked into the IRQ dispatcher peeks COM2 for 0x03 every IRQ and routes the IRQ's TrapFrame through the stop loop. **SMP rendezvous landed (2026-05-04)** — `arch::SmpStopBroadcastNmi`/`SmpStopReleaseNmi` plus a vector-2 freeze branch + per-CPU `gdb_snapshot_*` fields + `gdb_frozen_frame` pointer freeze peers on stop entry and resume them on `c`/`D`/`k`/`s`. **Multi-thread GDB visibility landed (2026-05-04)** — `qfThreadInfo`/`qsThreadInfo`/`qC`/`T`/`H g <tid>` expose every online CPU as a separate GDB thread; peers populate `g_peer_snapshots[cpu_id]` from `gdb_frozen_frame` on switch (read-only). **VSCode launch+tasks landed (2026-05-04)** — one-click attach via `.vscode/launch.json` + `tools/debug/vscode-{start,stop}-qemu.sh`. Remaining defers are mid-priority polish (writable peer regs, `vCont`, >4 HW watchpoints). | [knowledge/gdb-server-live-debug-v0.md](knowledge/gdb-server-live-debug-v0.md) | Observation + Pattern | Active | 2026-05-04 |
