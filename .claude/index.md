...
| **Live debug via in-kernel GDB stub v0** — RSP parser + qSupported / qXfer:features:read (24-reg target.xml) / g/G/m/M/H/c/s/Z0/z0/D/k packets; COM2 (0x2F8) wired via QEMU `-serial tcp::1234,server=on,wait=off`; trap-frame plumbing (#BP rolls RIP back, #DB clears TF) so step / break / continue / read+write registers + memory all work end-to-end. AI-friendly helpers: `tools/debug/duetos-gdb-attach.sh` (interactive) + `duetos-gdb-cmd.sh` (batched non-interactive — default emits `info reg` + `x/16i $rip` + `x/16xg $rsp` + `bt 10`). Build flags: `DUETOS_GDB_STUB` to wire COM2, `DUETOS_GDB_DEMO` to fire int3 right after IDT phase | [knowledge/gdb-stub-live-debug-v0.md](knowledge/gdb-stub-live-debug-v0.md) | Observation + Pattern | Active | 2026-05-04 |
