...
| Kernel & debug design recommendations — 18-item leverage-ordered plan | [knowledge/kernel-debug-recommendations-plan.md](knowledge/kernel-debug-recommendations-plan.md) | Plan | Active (A1 + A4 + A4-followup×2 + C2 + B1.1 + B1.2 + B1.3 + A3 + A3-followup HT-on-Process + A2 + A2-followup `now_ns` + A2-followup `realtime_ft` + D1 infra + D1-followup SpinLock-tag×7 + D5 landed; B2 SMP + Mutex/RwLock lockdep integration + concrete KObject subclasses deferred) | 2026-04-27 |
