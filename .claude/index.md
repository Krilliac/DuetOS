...
| Crash dump v0 — embedded symbol table + bracketed dump file (+ register-bit decoders + GPR symbolization + readable uptime/task labels + tree-wide hex log readability pass: PCI/NVMe/AHCI/xHCI/USB/PE/GPT/ext4/FAT32/Linux-signals/Win32-NTSTATUS + VA-region tags on cr2/rsp/rbp/rip + boot-time mm-map anchor + peer-CPU NMI snapshots + per-CPU held-locks dump + return-address-pointer scan + **page-walk for cr2/rip: 4-level walk via panic-safe `mm::SnapshotPageWalk`, emits PML4/PDPT/PD/PT entry + flags + leaf phys, names the failure level on NotPresent / non-canonical / out-of-direct-map — schema v3**) | [knowledge/crash-dump-v0.md](knowledge/crash-dump-v0.md) | Observation | Active | 2026-05-03 |
