...
| Kernel & debug design recommendations — 18-item leverage-ordered plan | [knowledge/kernel-debug-recommendations-plan.md](knowledge/kernel-debug-recommendations-plan.md) | Plan | Active (A1 full + A4 + A4-followup×3 + C1 zones + C2 + B1.1-B1.4 + B1-followup contention×3 + B1-followup AS RwLock + B1-followup timekeeper SeqLock + A3 full + A3-followup full IPC sextet (KMutex/KEvent/KSemaphore/KMailbox/KWaitable/KFile) + A2 + A2-followup×10 + D1 infra + D1-followup all globals + D1-followup `inspect lockdep` + D1-followup panic-on-invert + D1-followup per-CPU prep + D2 + D2-followup tracer dump+kind + D2-followup per-CPU prep + D3 + D3-followup perf dump + D3-followup PMU NMI wiring + D4 + D4-followup per-CPU prep + D5 + D5-followup×2 + D6 + D6-followup watch + D7 GDB stub scaffold + E1 probe + E2 investigation + E2-followup probe + E3 driver-domain + E3-followup×4 (soft-lockup + lockdep + event-trace + perf) landed; B2 SMP + SYS_MUTEX/EVENT/SEM migration + Linux fd / Win32 handle migration onto KFile + KPTI impl + CET enable + per-zone allocator + GDB serial RX wiring deferred) | 2026-04-28 |
