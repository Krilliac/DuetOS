...
| Crash dump v0 — embedded symbol table + bracketed dump file (+ register-bit decoders + GPR symbolization + readable uptime/task labels + tree-wide hex log readability pass: PCI/NVMe/AHCI/xHCI/USB/PE/GPT/ext4/FAT32/Linux-signals/Win32-NTSTATUS + VA-region tags on cr2/rsp/rbp/rip + boot-time mm-map anchor + peer-CPU NMI snapshots + per-CPU held-locks dump + return-address-pointer scan + page-walk for cr2/rip + **Architectural LBR snapshot via CPUID.7.0.EDX[19]: LbrInitBsp programs MSR_ARCH_LBR_CTL/DEPTH at boot, panic path freezes via LbrFreeze and dumps `from -> to` per entry symbolized; "(unavailable)" line on TCG/AMD/pre-Goldmont-Plus — schema v4**) | [knowledge/crash-dump-v0.md](knowledge/crash-dump-v0.md) | Observation | Active | 2026-05-03 |
