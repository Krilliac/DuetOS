...
| Crypto graduates to OS core + password-hash module v0 — `kernel/net/wireless/crypto/` → `kernel/crypto/` (namespace `duetos::crypto`); added Pbkdf2HmacSha256 + RFC 7914 KATs; new `kernel/security/password_hash.{h,cpp}` (PBKDF2-HMAC-SHA256 over 16-byte random salt + 100k default iterations + constant-time verify + 56-byte on-disk PasswordHashRecord struct) — KAT-driven, fail-closed against unknown algorithm/zero-iter, distinct-salts assertion verifies entropy pool is alive. Lays groundwork for the auth.cpp overhaul (one-call swap when a user table lands) | [knowledge/crypto-os-core-and-password-hash-v0.md](knowledge/crypto-os-core-and-password-hash-v0.md) | Decision + Pattern + Observation | Active — primitive in place; user-table on disk is the next bounded slice | 2026-05-03 |
