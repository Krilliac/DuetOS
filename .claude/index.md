...
| Kernel & debug design recommendations — 18-item leverage-ordered plan | [knowledge/kernel-debug-recommendations-plan.md](knowledge/kernel-debug-recommendations-plan.md) | Plan | Active (A1 + A1-followup all 11 phases (full migration complete) + A4 + A4-followup×3 + C2 + B1.1 + B1.2 + B1.3 + B1-followup contention×3 + A3 + A3-followup HT-on-Process + A3-followup full IPC quintuplet (KMutex/KEvent/KSemaphore/KMailbox/KWaitable) + A2 + A2-followup×9 (incl. TSC clocksource + tick wrapper + ALL call-site migrations done) + D1 infra + D1-followup all globals tagged + D1-followup `inspect lockdep` + D1-followup panic-on-invert + D2 + D4 + D5 + D5-followup preset + D5-followup smoke + D6 + D6-followup watch + E2 investigation + E2-followup mitigations probe landed; B2 SMP + SYS_MUTEX_*/EVENT_*/SEM_* migration + KPTI impl deferred) | 2026-04-28 |
