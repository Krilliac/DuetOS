...
| Kernel & debug design recommendations — 18-item leverage-ordered plan | [knowledge/kernel-debug-recommendations-plan.md](knowledge/kernel-debug-recommendations-plan.md) | Plan | Active (A1 + A1-followup all 11 phases + A1-followup _init_array + A1-followup KERNEL_INITCALL macro + A4 + A4-followup×3 + C1 zones + C2 + B1.1-B1.4 + B1-followup contention×3 + B1-followup AS RwLock + B1-followup timekeeper SeqLock + A3 + A3-followup HT-on-Process + A3-followup full IPC quintuplet + A2 + A2-followup×9 + D1 infra + D1-followup all globals + D1-followup `inspect lockdep` + D1-followup panic-on-invert + D1-followup per-CPU held-stack prep + D2 + D2-followup tracer dump+kind + D3 + D3-followup perf dump + D3-followup PMU NMI wiring + D4 + D4-followup per-CPU prep + D5 + D5-followup×2 + D6 + D6-followup watch + E1 probe + E2 investigation + E2-followup probe + E3 driver-domain landed; B2 SMP + SYS_MUTEX/EVENT/SEM migration + KPTI impl + CET enable + per-zone allocator deferred) | 2026-04-28 |
