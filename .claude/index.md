...
| **Live debug via in-kernel GDB server v0 — feature-complete** — RSP parser + qSupported / qXfer:features:read (24-reg target.xml) / g/G/m/M/H/c/s/Z0..Z4/z0..z4/D/k/vCont/vCont? + qfThreadInfo/qsThreadInfo/qC/T packets; COM2 (0x2F8) wired via QEMU `-serial tcp::1234,server=on,wait=off`; trap-frame plumbing (#BP rolls RIP back, #DB clears TF) so step / break / continue / read+write registers + memory all work end-to-end. **SW + HW breakpoints backed by `kernel/debug/breakpoints` subsystem**; install errors translate `BpError` → distinct GDB E1n codes (E12 = NoHwSlot for the recurring 5th-watchpoint case). AI-friendly helpers: `tools/debug/duetos-gdb-attach.sh` + `duetos-gdb-cmd.sh`; **VSCode one-click attach** via `.vscode/launch.json` + `tools/debug/vscode-{start,stop}-qemu.sh`. Build flags: `DUETOS_GDB_SERVER` (default-ON in x86_64-debug) + `DUETOS_GDB_DEMO`. **Async Ctrl-C stop, SMP NMI rendezvous, multi-thread visibility (peers as GDB threads with read+write registers via `Hg`+`G`+commit-on-release), and `vCont` resume verb all landed (2026-05-04).** v0 protocol surface complete; remaining items are real-hardware-only (null-modem attach, multi-thread step on peer threads). | [knowledge/gdb-server-live-debug-v0.md](knowledge/gdb-server-live-debug-v0.md) | Observation + Pattern | Active — feature-complete | 2026-05-04 |
