...
| Kernel & debug design recommendations — 18-item leverage-ordered plan | [knowledge/kernel-debug-recommendations-plan.md](knowledge/kernel-debug-recommendations-plan.md) | Plan | Active (A1 + A4 + A4-followup×3 + C2 + B1.1 + B1.2 + B1.3 + A3 + A3-followup HT-on-Process + A2 + A2-followup×3 + D1 infra + D1-followup SpinLock-tag×7 + D1-followup Mutex-tag + D1-followup RwLock-tag + D1-followup `inspect lockdep` + D5 + D5-followup preset + D6 + E2 investigation landed; B2 SMP + concrete KObject subclasses + KPTI impl deferred) | 2026-04-27 |
