...
| Crash dump v0 — embedded symbol table + bracketed dump file (+ register-bit decoders + GPR symbolization + readable uptime/task labels + tree-wide hex log readability pass: PCI/NVMe/AHCI/xHCI/USB/PE/GPT/ext4/FAT32/Linux-signals/Win32-NTSTATUS + VA-region tags on cr2/rsp/rbp/rip + boot-time mm-map anchor + peer-CPU NMI snapshots + per-CPU held-locks dump + return-address-pointer scan + page-walk for cr2/rip + Architectural LBR snapshot + per-task syscall trail + **per-process VM info: pid/name + pml4_phys/region count/budget + vmap span (min/max user VA + total page count) + loaded-DLL table (base..base+size, name resolved via PeExportsDllName); only emits when CurrentProcess() is non-null — schema v6**) | [knowledge/crash-dump-v0.md](knowledge/crash-dump-v0.md) | Observation | Active | 2026-05-03 |
