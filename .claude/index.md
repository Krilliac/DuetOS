...
| Function-branch NOP-patch attack v0 — synthetic cap-gate inline-asm target, 2-byte `je`→`90 90` patch, BYPASS LANDED + middle-`.text` placement reveal that `ComputeTextSpotHash`'s 4 KiB head/tail spot windows miss surgical mid-section modifications | [knowledge/branch-nop-attack-v0.md](knowledge/branch-nop-attack-v0.md) | Observation + Decision + Pattern | Active — `FailNoDetect` is the slice's payload; closes when a full-text or page-CRC detector lands | 2026-05-03 |
