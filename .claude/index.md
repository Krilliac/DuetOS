...
| Crash dump v0 — embedded symbol table + bracketed dump file (+ register-bit decoders + GPR symbolization + readable uptime/task labels + tree-wide hex log readability pass: PCI/NVMe/AHCI/xHCI/USB/PE/GPT/ext4/FAT32/Linux-signals/Win32-NTSTATUS + VA-region tags on cr2/rsp/rbp/rip + boot-time mm-map anchor + peer-CPU NMI snapshots + per-CPU held-locks dump + return-address-pointer scan + page-walk for cr2/rip + Architectural LBR snapshot + **per-task syscall trail (last 8): inline ring on `sched::Task`, populated by RAII guard in native int-0x80 dispatcher + explicit hook in Linux dispatcher, dumped on panic only when non-empty (kernel threads stay silent); SyscallTrailSelfTest verifies record/wrap/clamp at boot — schema v5**) | [knowledge/crash-dump-v0.md](knowledge/crash-dump-v0.md) | Observation | Active | 2026-05-03 |
