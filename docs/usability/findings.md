...
|----|---------|----------|---------|----------|
| F-010 | calculator | Medium | no decimal-point button (grid is 7-9/+ 4-6/- 1-3/* C 0 = /) — can't enter fractions | calculator-open.png | **FILED.** The whole engine is signed `i64` (`ReadDisplayAsI64`/`SetDisplayI64`/`ApplyPending`); a `.` and `1/4 = 0.25` need a fixed-point/float rework across the TU — a rewrite, not an extension. Filed to `wiki/reference/Roadmap.md` → "Usability campaign — app gaps". | **filed** |
...
| F-051 | calculator | Low | large-font decimal display is not clipped to the display strip — a 9-digit value (`987654321`) renders past the window's right border onto the wallpaper | calc-digits.png | surfaced while disproving F-011; the `AppLabel(Display)` draws at full glyph width with no clip rect / no auto-shrink for long strings | open |
| F-012 | calculator | Medium | no CE (Clear Entry) distinct from C (rubric wants both) | calculator-open.png | rubric wants both full-reset (C) and clear-current-operand (CE); engine has only `HandleClear` (full reset). Bounded but the on-screen 4×4 grid is full (16 keys) — needs a layout slot or a keyboard-only CE. | open |
...
| F-030 | settings/sound | High | Sound panel only toggles UI-cue mute + PC-speaker beep — no master volume slider, no HDA output volume | settings-08-snd.png | **FILED.** The audio backend (`kernel/subsystems/audio/audio_backend.*`) is a v0 fixed mixer with no gain stage; HDA exposes no codec amp control. Needs an `AudioSetMasterGain` API (software-gain is the smaller lift). Filed to Roadmap → "Usability campaign — app gaps". | **filed** |
