...
2. **Auth and privilege are kernel-owned.** A process's authority is its durable caps plus unexpired broker leases, masked by a monotonic grant ceiling; `Process::cap_lock` protects that state and kernel cap gates consume an atomic effective snapshot. Win32-shaped token calls use kernel helpers: disable clears live bits reversibly, `SE_PRIVILEGE_REMOVED` permanently lowers the ceiling, and enabling a missing cap must pass through the elevation broker. The broker is the sole controlled post-spawn grant bridge and may install only a positive-duration lease after role/password policy succeeds; integrity levels and ACL-shaped probes remain facades.
