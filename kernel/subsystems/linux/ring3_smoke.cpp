...
void SpawnRing3LinuxElfSmoke()
{
    ...
    if (n == 0)
    {
        core::DebugPanicOrWarn("linux/smoke", "BuildLinuxElf scratch too small");
        return;
    }
...
//   sys_chmod(0xdeadbeef, 0x755, 0, 0, 0, 0) -> MISS (TU doesn't
//     handle it either — boot log gets one [linux-miss] line with
//     name="chmod" and all six arg registers)
//   exit_group(0x42)
// The boot log shows both [translate] lines + one [linux-miss]
// line; re-run shows the same lines, so grep "[linux-miss]" reveals
// every Linux syscall the running PE/ELF actually reaches that the
// kernel doesn't implement (a live "what do we still need?" list).
constexpr u8 kTranslatePayload[] = {
    ...
    0x31, 0xD2,                   // xor edx, edx
    0x0F, 0x05,                   // syscall
    // sys_chmod(0xDEADBEEF, 0x755)  — genuine TU miss
    0xB8, 0x5A, 0x00, 0x00, 0x00, // mov eax, 90
    0xBF, 0xEF, 0xBE, 0xAD, 0xDE, // mov edi, 0xDEADBEEF
    0xBE, 0x55, 0x07, 0x00, 0x00, // mov esi, 0x755
    0x0F, 0x05,                   // syscall
    // exit_group(0x42)
