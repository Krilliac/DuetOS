...
use crate::format::{
    BLOCK_SIZE, JOURNAL_BLOCKS, JOURNAL_DESCRIPTOR_MAGIC, JOURNAL_STATE_COMMITTED, JOURNAL_STATE_EMPTY, MAX_JOURNAL_OPS,
};
...
fn write_descriptor<D: BlockDevice + ?Sized>(dev: &mut D, jlba: u32, d: &JournalDescriptor) -> FsResult<()> {
    let mut block = [0u8; BLOCK_SIZE];
    let raw = unsafe {
...
pub fn apply<D: BlockDevice + ?Sized>(dev: &mut D, jlba: u32, txn_id: u32, ops: &[(u32, &[u8])]) -> FsResult<()> {
    if ops.is_empty() {
        return Ok(());
    }
...
#[allow(dead_code)]
pub fn peek_descriptor<D: BlockDevice + ?Sized>(dev: &mut D, jlba: u32) -> FsResult<JournalDescriptor> {
