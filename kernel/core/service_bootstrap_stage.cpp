...
ServiceBootstrapStageResultV1 PrepareStagedRow(const ServiceManifestServiceV1& service,
                                               const ServiceExecutableTransferSnapshotV1& transfer, u32 manifest_index,
                                                                ...
                                                                u32 slot_capacity)
{
    ...
    for (u32 order_index = 0; order_index < manifest.plan->topological_count; ++order_index)
    {
        ...
        result = PrepareStagedRow(service, transfer, manifest_index,
                                                             ...
                                                             const ServiceBootstrapSlotStorageV1* replacement)
{
    ...
    result = PrepareStagedRow(service, transfer, manifest_index, replacement_memory_object, *replacement,
                              selected->activation_generation, admission_first_identity, &prepared);
    if (result.status != ServiceBootstrapStageStatus::Ok)
        return result;
