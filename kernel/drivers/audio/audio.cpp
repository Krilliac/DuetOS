/*
 * DuetOS — Audio shell: PCI discovery + dispatch to per-kind drivers.
 *
 * Walks the PCI device cache for class 0x04 (multimedia) and
 * registers each controller. HDA (subclass 0x03) is the only kind
 * whose v0 driver actually does any bring-up work; AC'97 and
 * legacy controllers are logged as "present but no driver".
 *
 * The HDA driver itself lives in `drivers/audio/hda.cpp`. This
 * file used to host both shell + HDA code in a single 700-line
 * blob; the split keeps both files under the bloat threshold and
 * lets the HDA driver grow stream-routing / playback support
 * without crowding the shell.
 */
...
void AudioInit()
{
    ...
    for (u64 i = 0; i < g_ac_count; ++i)
    {
        ...
        {
            // Only the first HDA controller gets brought up — the
            // hda module's static state is single-controller.
            if (!hda::IsBroughtUp())
            {
                auto r = hda::BringUp(g_acs[i]);
                if (!r.has_value())
