...
const char* const kCommandSet[] = {
    ...
    "curl",       "sha256sum",   "shasum",    "base64",    "xxd",         "metrics",      "trace",    "read",
    ...
    "dfix",       "drshd",       "pe-triage", "caplog",    "live-update", "fault-inject",
};
...
void Dispatch(char* line)
{
    ...
    if (StrEq(cmd, "panic-test"))
    {
        CmdPanicTest(argc, argv);
        return;
    }
    if (StrEq(cmd, "uptime"))
    {
    ...
    if (StrEq(cmd, "fault-inject"))
    {
        // Deliberate panic / kernel #PF / slab OOM trigger. Gated on
        // kCapDiag because two of the three classes halt the box —
        // a sandboxed shell session must not reach this. The kernel
        // shell pseudo-process gets kCapDiag via the broker's elevate
        // flow against the root role.
        if (!RequireCap(::duetos::core::kCapDiag, "FAULT-INJECT"))
            return;
        CmdFaultInject(argc, argv);
        return;
    }
    if (StrEq(cmd, "instr"))
    {
