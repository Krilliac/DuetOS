...
void CmdHelp()
{
    ...
    ConsoleWriteln("  RESMON       CPU + MEM + BOX UTILISATION (ONE SNAPSHOT)");
...
void Dispatch(char* line)
{
    ...
    if (StrEq(cmd, "dintro"))
    {
        // Same exposure model as `dfix` — the introspect snapshot
        // is derived from the journal the kernel has already chosen
        // to make visible, plus the prior boot's KERNEL.F0 file
        // which is FS-readable by anything with kCapFsRead anyway.
        CmdDintro(argc, argv);
        return;
    }
    if (StrEq(cmd, "selfthink"))
    {
        // Cross-subsystem self-portrait + causal-chain ring. Pure
        // read paths over counters every other observability
        // command already exposes (resmon / ps / health / dfix /
        // dintro / probe). No cap gate — symmetric with the rest
        // of the read surface.
        CmdSelfthink(argc, argv);
        return;
    }
    ...
    if (StrEq(cmd, "slotinfo"))
    {
        CmdSlotinfo();
        return;
    }
    if (StrEq(cmd, "bootslot"))
    {
        // bootslot's internal RequireAdmin gate sits per-subcommand
        // so `bootslot` with no args (or with `--help` if added)
        // can print usage without forcing a login. The state-
        // changing subcommands gate themselves.
        CmdBootslot(argc, argv);
        return;
    }
